...
class ImagesFromUrls {
	...
	public function __construct(array $urls, string $title, string $privacy_level = Connector::PRIVACY_PRIVATE, bool $comments_off = false, bool $auto_add_music = false, bool $is_brand_content = false, bool $is_brand_organic = false) {
		if (!Connector::isValidPrivacyLevel($privacy_level)) {
			throw new RuntimeException('TikTok Invalid Privacy Level Provided: '.$privacy_level.". Must be: ".implode(', ', Connector::VALID_PRIVACY));
		}
		if (!count($urls)) {
			throw new RuntimeException('Please provide at least 1 image URL to Publish');
		}
		foreach ($urls as $url) {
	...
	public function publish(Connector $tk): PublishInfo
    {
		...
		if (!$CreatorQuery->hasPrivacyOption($this->getPrivacyLevel())) {
			throw new RuntimeException('TikTok Error: This Creator cannot publish with the privacy level '.implode(', ', $CreatorQuery->getPrivacyOptions()));
		}
	...
	public function publishWithoutChecks(Connector $tk): PublishInfo
    {
		try {
			...
			$res = $tk->postWithAuth(Connector::BASE_PHOTO_PUBLISH, $data);
			if (!$res) {
	 ...
	 */
	public function getAutoAddMusic(): bool
    {
